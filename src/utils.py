...
def rel_co_matrix(triples, rel_num, norm=False):
    # co_occur_matrix = [[0]*rel_num for i in range(rel_num)]
    M = {}
    # rel_count = {}
    ind, val = [], []
    for (h, r, t) in triples:
        ...
        M[r].add(t)
        # rel_count[r] += 1
    M_order = sorted(M.items(), key=lambda x: x[0], reverse=False)  # 按关系ID排序
    for i in range(rel_num):
        ind.append((i, i))
        val.append(1)
        # co_occur_matrix[i][i] = rel_count[i]
        for j in range(i+1, rel_num):
            en_len = len(list(set(M_order[i][1]).intersection(set(M_order[j][1]))))
            # co_occur_matrix[i][j] = en_len
            # co_occur_matrix[j][i] = co_occur_matrix[i][j]
            ind.append((i, j))
            ind.append((j, i))
            val.append(en_len)
            val.append(en_len)
    if norm:
        ...
        adj = sp.coo_matrix((val, (ind[:, 0], ind[:, 1])), shape=(rel_num, rel_num), dtype=np.float32)
        return sparse_mx_to_torch_sparse_tensor(normalize_adj(adj))
    else:
        M = torch.sparse_coo_tensor(torch.LongTensor(ind).t(), torch.FloatTensor(val), torch.Size([rel_num, rel_num]))
        return M


def get_nhop_co_matrix(triples, rel_num, ent_num, norm=False):
    e_in, e_out = {}, {}
    M = {}
    for (h, r, t) in triples:
        if r not in M:
    ...
    for ent in range(ent_num):
        if len(e_in[ent]) != 0 and len(e_out[ent]) != 0:
            for v in e_in[ent]:
                for v_o in e_out[ent]:
                    M[v].add(v_o)

    for (rel, out) in M.items():
        if len(out) != 0:
            for id in out:
                ind.append((rel, id))
                val.append(1)
...
def rel_in_out_co_matrix(triples, rel_num, norm=False):
    # co_occur_matrix = [[0]*rel_num for i in range(rel_num)]
    M_in = {}
    M_out = {}
    # rel_count = {}
    ...
    for (h, r, t) in triples:
        ...
        M_in[r].add(t)
        M_out[r].add(h)
    M_in_order = sorted(M_in.items(), key=lambda x: x[0], reverse=False)
    M_out_order = sorted(M_out.items(), key=lambda x: x[0], reverse=False)
    ...
    for i in range(rel_num):
        ...
        out_val.append(1)
        for j in range(i+1, rel_num):
            in_en_len = len(list(set(M_in_order[i][1]).intersection(set(M_in_order[j][1]))))
            out_en_len = len(list(set(M_out_order[i][1]).intersection(set(M_out_order[j][1]))))
            in_ind.append((i, j))
            in_ind.append((j, i))
            in_val.append(in_en_len)
            in_val.append(in_en_len)
            out_ind.append((i, j))
            out_ind.append((j, i))
            out_val.append(out_en_len)
            out_val.append(out_en_len)

    if norm:
        in_ind = np.array(in_ind, dtype=np.int32)
        in_val = np.array(in_val, dtype=np.float32)
        out_ind = np.array(out_ind, dtype=np.int32)
        out_val = np.array(out_val, dtype=np.float32)
        in_r_adj = sp.coo_matrix((in_val, (in_ind[:, 0], in_ind[:, 1])), shape=(rel_num, rel_num), dtype=np.float32)
        out_r_adj = sp.coo_matrix((out_val, (out_ind[:, 0], out_ind[:, 1])), shape=(rel_num, rel_num), dtype=np.float32)
        sp_in_adj = sparse_mx_to_torch_sparse_tensor(normalize_adj(in_r_adj))
        sp_out_adj = sparse_mx_to_torch_sparse_tensor(normalize_adj(out_r_adj))
        return sp_in_adj, sp_out_adj
    else:
        in_adj = torch.sparse_coo_tensor(torch.LongTensor(in_ind).t(), torch.FloatTensor(in_val), torch.Size([rel_num, rel_num]))
        out_adj = torch.sparse_coo_tensor(torch.LongTensor(out_ind).t(), torch.FloatTensor(out_val),
                                          torch.Size([rel_num, rel_num]))
        return in_adj, out_adj


def get_path_adj(triples, rel_num, ent_num, norm=False):
    e_in, e_out = {}, {}
    M_rev = {}
    for (h, r, t) in triples:
        if r not in M_rev:
    ...
    ind, val = [], []
    path_graph_rev = [[0 for i in range(rel_num)] for j in range(rel_num)]
    for ent in range(ent_num):
        if len(e_in[ent]) != 0 and len(e_out[ent]) != 0:
            for v in e_out[ent]:
                for v_o in e_in[ent]:
                    M_rev[v].append(v_o)
    for (rel, out) in M_rev.items():
        if len(out) != 0:
            for id in out:
                path_graph_rev[rel][id] += 1
                path_graph_rev[id][rel] += 1
                ind.append((rel, id))
                val.append(1)
    for i in range(rel_num):
        path_graph_rev[i][i] += 1
        ind.append((i, i))
        val.append(1)
    path_graph_rev = torch.Tensor(path_graph_rev)
    path_graph_rev = torch.nn.functional.normalize(path_graph_rev, dim=1).numpy()
